...
def generate_captions_for_frontiers(
    ...
    sys_prompt = (
        ...
        "BVF1: <description>\n\nBVF2: <description>\n\n…\n\nCVF1: <description>\n\nCVF2: <description>\n\n…\n\n"
        "RULE: Each description is 4–6 sentences. Focus on visual elements relevant to solving the question and guiding exploration. "
    ...
    # Attach BVFs with labels BVF1:, BVF2:, ...
    for i, rel in enumerate(initial_rels, start=1):
        content.append((f"BVF{i}:",))
        _add_img(rel)
    # Attach CVFs with labels CVF1:, CVF2:, ...
    for j, rel in enumerate(detail_rels, start=1):
        ...
        for question_id, qinfo in qdict.items():
            ...
            for step_key, step_info in steps.items():
                ...
                try:
                    ...
                    if chosen_l0 and isinstance(chosen_l0, str) and chosen_l0.startswith("frontier/"):
                        initial_key_for_details = chosen_l0.split("/", 1)[1]
                    detail_rels = frontier.get(initial_key_for_details, []) if initial_key_for_details else []

                    # Indices (1-based) for selected items
                    def _idx1(rel, rels):
                        try:
                            return str(rels.index(rel) + 1) if rel and rel in rels else "NA"
                        except Exception:
                            return "NA"

                    selected_l0_index = _idx1(chosen_l0, initial_rels)
                    selected_l1_index = _idx1(chosen_l1, detail_rels)
...
def main():
    ...
    parser.add_argument("--strategy", default="sim", choices=["sim", "random"], help="Candidate selection strategy: sim or random")
    parser.add_argument("--top_k", type=int, default=1, help="Number of candidates per frontier (>=1)")
