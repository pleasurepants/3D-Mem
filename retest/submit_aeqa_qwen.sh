...
for SEED in 19 32 568 82; do
  ...
  sbatch <<EOF
#!/bin/bash
#SBATCH --job-name=q_2sl_${SEED}
#SBATCH --nodes=1
#SBATCH --gres=gpu:2
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=10-00:00:00
#SBATCH --exclude=worker-minor-1,worker-minor-3,worker-minor-4,worker-minor-5,worker-minor-6,worker-1,worker-2,worker-3,worker-4,worker-6,worker-7,worker-8,worker-9
#SBATCH --output=/home/wiss/zhang/code/openeqa/3D-Mem/slurm/qwen/2stage_listwise/2stage_listwise-${SEED}-%j.out
