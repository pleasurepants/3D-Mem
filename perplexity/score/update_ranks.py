#!/usr/bin/env python3
"""
根据average分数重新划分rank为Low、Medium、High三组
"""
...
def update_ranks(input_file, output_file):
    """更新rank字段"""
    # 备份原文件
...
def verify_ranks(file_path):
    ...
    for rank, scores in rank_stats.items():
        if scores:
            print(f"{rank}组: {len(scores)}个样本, 分数范围: {min(scores):.3f} - {max(scores):.3f}, 平均: {sum(scores)/len(scores):.3f}")
        else:
            print(f"{rank}组: 0个样本")

def main():
    input_file = "traj_abs_format_score_rank.json"
    output_file = "traj_abs_format_score_rank_updated.json"
    
    print("正在更新rank字段...")
    stats = update_ranks(input_file, output_file)
