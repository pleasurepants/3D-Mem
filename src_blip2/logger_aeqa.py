...
from src_blip2.tsdf_planner import TSDFPlanner, Frontier, SnapShot
...
class Logger:
    def __init__(
        ...
        assert self.n_total == len(self.gpt_answer_list), f"{self.n_total} != {len(self.gpt_answer_list)}"
    ...
    def save_results(self):
        ...
        # assert len(self.success_list) == len(self.gpt_answer_list) wrong in the case of failure
        print("len(success_list)", len(self.success_list))
        print("len(fail_list)", len(self.fail_list))
        print('len(gpt_answer_list)', len(self.gpt_answer_list))
        print('len(n_filtered_snapshots_list)', len(self.n_filtered_snapshots_list))
        print('len(n_total_snapshots_list)', len(self.n_total_snapshots_list))
        print('len(n_total_frames_list)', len(self.n_total_frames_list))
        assert (len(self.success_list) + len(self.fail_list)) == len(self.gpt_answer_list)
