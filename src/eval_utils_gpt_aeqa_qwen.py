...
def format_frontier_multiscore_prompt(
    ...
    if egocentric_img is not None:
        content.append(("Current agent egocentric view:", egocentric_img))
        content.append((" ",))
    content.append(("The selected Frontier image:", chosen_frontier_img))
    content.append((" ",))
    content.append(("For the selected Frontier, answer each question below with only Yes or No:",))
    for i, dim in enumerate(dimension_names, 1):
        content.append((f"Q{i}: {dim}",))
    content.append(("Write your five answers in five lines, only Yes or No, nothing else.",))
