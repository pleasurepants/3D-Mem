...
def _load_ppl_rank_qids(cfg):
    ...
    ppl_rank_file = getattr(cfg, "ppl_rank_file", "perplexity/traj_abs_format_ppl_rank.json")
...
def _load_vector_store(cfg):
    ...
    try:
        ...
        png = _load_one('png', png_subdir_suffix)
...
def _rank_by_vectors_for_question(current_question: str, store: dict, top_k: int = 5, rrf_k: int = 60):
    ...
    try:
        ...
        import numpy as _np
        def _cos_sim(v, M):
            return (M @ v.astype(_np.float32))
        text_encoder = str((store['question']['enc'] or {}).get('text_encoder', 'clip')).lower()
        sims_img = None
        if text_encoder == 'clip':
            sims_img = _cos_sim(qv, store['png']['emb'])
        sims_txt = _cos_sim(qv, store['question']['emb'])
...
def explore_step(
    ...
    if idx0 not in step['layer0_to_layer1']:
        ...
        layer1_indices = step['layer0_to_layer1'][idx0]
