        ...
        while retry_count < max_tries:
            try:
                completion = client.chat.completions.create(
                    model="gpt-4o",  # gpt-4o-qwen-minicpm-qwen
...
def main(cfg, start_ratio=0.0, end_ratio=1.0):
    ...
    clip_model, _, clip_preprocess = open_clip.create_model_and_transforms(
        "ViT-H-14", pretrained="laion2b_s32b_b79k"  # "ViT-H-14", "laion2b_s32b_b79k"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ppl_rank_file", help="path to ppl_rank json file", default="perplexity/traj_abs_format_ppl_rank.json", type=str)
    ...
    # experience_training_status parameter (only set if provided via CLI, not from cfg file)
