import os
# 避免 Pillow 加载 LERC 插件触发系统 libstdc++ 版本冲突
...
def find_frontier_pngs(src_root: str) -> List[Dict]:
    """
    遍历 src_root 下的所有 question_id 目录，收集其中 frontier 文件夹内的 PNG。
    返回列表，每项包含：
      { 'qid': str, 'png_path': str }
    """
    items: List[Dict] = []
...
def load_questions_map(qpath: str) -> Dict[str, str]:
    """
    从指定路径读取 aeqa_questions-168.json，构建 {question_id: question_text}
    若读取失败，返回空字典。
    """
    try:
...
def load_clip(clip_model: str, device: str, open_clip_pretrained: str = None, open_clip_tokenizer_model: str = None):
    """
    优先用 transformers 加载 CLIP，失败则尝试 open_clip。
    返回 (preprocess_images_fn, encode_images_fn, encode_texts_fn, img_dim, txt_dim)
    preprocess_images_fn: (List[PIL.Image]) -> model inputs (dict 或 tensor)
    encode_images_fn: (inputs) -> np.ndarray [N, D]，L2 标准化
    encode_texts_fn: (List[str]) -> np.ndarray [N, D]，L2 标准化
    """
    # transformers 路径
    try:
    ...
    try:
        import open_clip
        import torch
        # 解析 model 与 pretrained
        model_name = clip_model
        pretrained = open_clip_pretrained or 'openai'
        # 兼容常见传参
        # - 若传入类似 'laion/CLIP-ViT-H-14-laion2B-s32B-b79K'，则拆分出 'ViT-H-14' 与 'laion2B-s32B-b79K'
...
def main():
    ...
    parser.add_argument('--src_root', type=str, required=True, help='源数据根目录，例如 /anvme/.../qwen-exp-168/')
    parser.add_argument('--dst_root', type=str, required=True, help='输出根目录，例如 /anvme/.../qwen-exp-168/retrieve')
    parser.add_argument('--clip_model', type=str, default='openai/clip-vit-base-patch32')
    parser.add_argument('--open_clip_pretrained', type=str, default=None, help='open_clip 的 pretrained 名称或本地权重路径（.bin）')
    parser.add_argument('--open_clip_tokenizer_model', type=str, default=None, help='open_clip 的 tokenizer 模型名，默认为 clip_model')
    parser.add_argument('--sbert_model', type=str, default='sentence-transformers/all-MiniLM-L6-v2')
    parser.add_argument('--text_encoder', type=str, default='clip', choices=['clip', 'sbert'], help='question 通道使用的文本编码器')
    parser.add_argument('--faiss_factory_img', type=str, default='Flat')
    parser.add_argument('--faiss_factory_txt', type=str, default='Flat')
    parser.add_argument('--copy_images', action='store_true', help='是否将 PNG 复制到目标 png 目录')
    parser.add_argument('--questions_path', type=str, default='/home/hpc/v100dd/v100dd12/code/3D-Mem/data/aeqa_questions-168.json', help='问题文件 JSON 的绝对路径')
    ...
    # 将 PNG 直接放在 dst_png_root 下（不再建 images 子目录）
