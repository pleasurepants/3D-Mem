#!/bin/bash
# Submit random top-3 runs for four seeds (with local vLLM server)
...
for SEED in 13 568 82 19; do
  YAML=${CURSOR_DIR}/qwen_${SEED}_random_top3.yaml
  sbatch <<EOF
#!/bin/bash
#SBATCH --job-name=q-ee-random-top3-${SEED}
#SBATCH --nodes=1
#SBATCH --gres=gpu:a40:2
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=16
#SBATCH --time=24:00:00
#SBATCH --output=${ROOT}/slurm/ee/qwen-random-top3-${SEED}-%j.out
